<template>
  <div v-if="isAuthenticated" class="flex h-screen bg-slate-50 overflow-hidden">
    ...
    <div class="flex-1 flex flex-col min-w-0 overflow-hidden">
      ...
      >
        <div v-if="!isAuthenticated || rolesLoading" class="absolute inset-0 z-[60] bg-slate-50/80 backdrop-blur-sm flex items-center justify-center">
            <div class="flex flex-col items-center">
                <div class="w-12 h-12 border-4 border-indigo-200 border-t-indigo-600 rounded-full animate-spin mb-4"></div>
                <p class="text-slate-500 font-medium animate-pulse">Memuat Data Role...</p>
            </div>
        </div>
      </transition>
      ...
      <main v-if="isAuthenticated" class="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8">
        <!-- Header Section -->
        <div class="mb-8">
          <h1 class="text-2xl font-bold text-slate-900">Manajemen Role</h1>
          <p class="text-slate-500">Kelola role dan hak akses user STIM YKPN.</p>
        </div>

        <!-- Toolbar Section -->
        <div class="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div class="w-full sm:max-w-md">
            <AppInput v-model="searchQuery" placeholder="Cari nama role..." id="search-role">
              <template #icon>
          ...
          <div class="flex flex-wrap items-center gap-3">
            ...
            >
              <div v-if="selectedRoles.length > 0" class="flex items-center gap-3 z-10">
        ...
        <div class="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden flex flex-col">
          <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse min-w-[800px]">
              <thead>
                <tr class="bg-slate-50 border-b border-slate-100">
                  <th class="px-6 py-4 w-12">
                    <input type="checkbox" @change="toggleSelectAll" :checked="selectedRoles.length === paginatedRoles.filter(r => r.name !== 'administrator').length && paginatedRoles.filter(r => r.name !== 'administrator').length > 0" class="w-4 h-4 rounded border-slate-300 text-indigo-600 focus:ring-indigo-500 cursor-pointer transition-colors">
                  </th>
              ...
              <tbody class="divide-y divide-slate-100">
                <tr v-for="item in paginatedRoles" :key="item.id" class="hover:bg-slate-50 transition-colors group">
                  <td class="px-6 py-4">
                    <input v-if="item.name !== 'administrator'" type="checkbox" :value="item.id" v-model="selectedRoles" class="w-4 h-4 rounded border-slate-300 text-indigo-600 focus:ring-indigo-500 cursor-pointer transition-colors">
                  ...
                  <td class="px-6 py-4">
                    <p class="text-sm text-slate-600 truncate max-w-xs">{{ item.description || '-' }}</p>
                  </td>
                  ...
                  <td class="px-6 py-4 text-right">
                    <div class="flex items-center justify-end gap-2">
                        <a :href="`/roles/edit/${item.id}`" class="inline-flex items-center gap-1 px-3 py-1.5 text-xs font-semibold text-amber-600 bg-amber-50 hover:bg-amber-100 rounded-lg transition-colors">
                  ...
                  <td colspan="5" class="px-6 py-12 text-center text-slate-500 text-sm">
                    <div class="flex flex-col items-center">
                        <div class="w-16 h-16 bg-slate-100 rounded-full flex items-center justify-center mb-3">
                            <svg xmlns="http://www.w3.org/2000/svg" class="w-8 h-8 text-slate-300" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z" /></svg>
                        </div>
                        <p class="font-medium text-slate-600">Tidak ada data role yang ditemukan.</p>
                        <p class="text-xs text-slate-400 mt-1">Gunakan kata kunci lain atau tambahkan role baru.</p>
                    </div>
                  </td>
                </tr>
              </tbody>
          ...
          <div v-if="totalPages > 1" class="px-6 py-4 border-t border-slate-100 flex flex-col sm:flex-row items-center justify-between bg-slate-50/50 gap-4">
            <div class="text-sm text-slate-500 text-center sm:text-left">
              ...
              dari <span class="font-bold text-slate-900">{{ totalItems }}</span> role
            </div>
        ...
        <div v-if="showDeleteModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm transition-opacity">
          <div class="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden transform transition-all translate-y-0 scale-100">
            <div class="p-6">
              ...
              <h3 class="text-xl font-bold text-slate-900 mb-2">Hapus {{ isBulkDelete ? selectedRoles.length + ' Role Terpilih' : 'Role' }}?</h3>
              <p class="text-slate-500 text-sm">
                Tindakan ini tidak dapat dibatalkan. Role yang dihapus akan hilang dari sistem secara permanen.
              </p>
            </div>
...
const filteredRoles = computed(() => {
  if (!searchQuery.value) return allRoles.value;
  const q = searchQuery.value.toLowerCase();
  return allRoles.value.filter(n => 
...
const paginatedRoles = computed(() => {
  ...
  return filteredRoles.value.slice(offset, offset + limit.value);
});
...
const toggleSelectAll = (e) => {
  if (e.target.checked) selectedRoles.value = paginatedRoles.value.filter(r => r.name !== 'administrator').map(n => n.id);
...
const executeDelete = async () => {
  ...
  try {
    ...
    if (isBulkDelete.value) {
      await axios.post('/api/roles/bulk-delete', { ids: selectedRoles.value }, {
        headers: { Authorization: `Bearer ${token}` }
      });
      selectedRoles.value = [];
    } else {
      await axios.delete(`/api/roles/${itemToDeleteId.value}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
    }
    fetchRoles();
