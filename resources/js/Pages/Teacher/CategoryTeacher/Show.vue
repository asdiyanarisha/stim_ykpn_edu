      ...
      <main class="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8">
        ...
        <div class="max-w-4xl mx-auto mb-6">
          <button @click="goBack" class="inline-flex items-center gap-2 text-sm font-medium text-slate-500 hover:text-indigo-600 transition-colors cursor-pointer group">
            ...
            Kembali ke Daftar Kategori Dosen
          </button>
        </div>
        ...
        >
          <div v-if="!isAuthenticated || isLoading" class="absolute inset-0 z-[60] bg-slate-50/80 backdrop-blur-sm flex items-center justify-center">
              <div class="flex flex-col items-center">
                  ...
                  <p class="text-slate-500 font-medium animate-pulse">Memuat Detail Kategori...</p>
              </div>
          </div>
        </transition>

        <div v-if="category.id" class="max-w-4xl mx-auto space-y-8 pb-12">
          ...
          <div class="bg-white rounded-[2rem] shadow-sm border border-slate-100 overflow-hidden">
            <div class="p-8 md:p-12">
                ...
                <div class="mb-10">
                    <h1 class="text-3xl md:text-4xl font-extrabold text-slate-900 leading-tight">{{ category.title || 'Tanpa Judul' }}</h1>
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div class="p-4 bg-slate-50 border border-slate-100 rounded-2xl">
                        ...
                        <p class="font-bold text-slate-700">{{ formatFullDate(category.created_at) }}</p>
                    </div>
                    <div class="p-4 bg-slate-50 border border-slate-100 rounded-2xl">
                        <p class="text-[10px] uppercase tracking-widest font-bold text-slate-400 mb-1">Terakhir Diperbarui</p>
                        <p class="font-bold text-slate-700">{{ formatFullDate(category.updated_at) }}</p>
                    </div>
                </div>
            </div>
          </div>
          ...
          <div class="flex justify-between items-center bg-white p-6 rounded-[1.5rem] shadow-sm border border-slate-100">
            <div class="text-sm text-slate-400 italic">
                Informasi detail kategori dosen.
            </div>
            <div class="flex gap-3">
                <a :href="`/masterData/teacher/category/edit/${category.id}`">
...
const fetchCategoryDetail = async () => {
  ...
  try {
    ...
    const response = await axios.get(`/api/category-teachers/${id}`, {
    ...
    if (error.response?.status === 404) {
       window.location.href = '/masterData/teacher/category';
    }
  } finally {
