        ...
        <div class="max-w-4xl mx-auto mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <div class="flex items-center gap-2 text-sm text-slate-500 mb-2">
              <a href="/public-files" class="hover:text-indigo-600 transition-colors">Manajemen File Publik</a>
              <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" /></svg>
              <span class="text-slate-900 font-medium">{{ isEditMode ? 'Ubah File' : 'Tambah File' }}</span>
            </div>
            <h1 class="text-2xl font-bold text-slate-900">{{ isEditMode ? 'Ubah Informasi File Publik' : 'Formulir File Publik Baru' }}</h1>
            <p class="text-slate-500">{{ isEditMode ? 'Perbarui informasi file publik yang sudah ada.' : 'Unggah dan beri informasi file publik baru untuk STIM YKPN.' }}</p>
          </div>
          ...
          <form @submit.prevent="handleSubmit" class="space-y-6">
            ...
            <div class="bg-white rounded-3xl p-8 shadow-sm border border-slate-100">
              ...
              <div class="flex flex-col items-center justify-center border-2 border-dashed border-slate-200 rounded-3xl p-12 hover:border-indigo-400 hover:bg-indigo-50/30 transition-all group relative overflow-hidden cursor-pointer" @click="$refs.fileInput.click()">
                <input type="file" ref="fileInput" class="hidden" accept=".pdf,.doc,.docx,.ppt,.pptx,.zip,.rar,.xls,.xlsx,image/*" @change="handleFileUpload">
                ...
                <template v-if="!fileSelectedName">
                  ...
                  <p class="text-slate-400 text-sm mt-1">Format: PDF, DOC, ZIP, XLS, Gambar (Maks. 10MB)</p>
                </template>
            ...
            <div class="bg-white rounded-3xl p-8 shadow-sm border border-slate-100">
              <h2 class="text-lg font-bold text-slate-800 mb-6 flex items-center gap-2">
                ...
                Informasi File Publik
              </h2>

              <div class="space-y-6">
                <AppInput v-model="form.title" label="Judul File" placeholder="Masukkan judul file publik..." id="file-title" :error="errors.title" />

                <div class="space-y-1">
                    <label class="block text-sm font-bold text-slate-700 mb-1.5 ml-1">Deskripsi File</label>
                    <textarea 
                        ...
                        placeholder="Berikan deskripsi singkat mengenai file ini..."
                    ></textarea>
            ...
            <div class="bg-white rounded-3xl p-6 shadow-sm border border-slate-100 flex items-center justify-end gap-4">
              ...
              <AppButton variant="primary" size="md" type="submit" :disabled="isSubmitting">
                ...
                {{ isSubmitting ? (isEditMode ? 'Menyimpan...' : 'Menambahkan...') : (isEditMode ? 'Simpan Perubahan' : 'Simpan File') }}
              </AppButton>
            </div>
          </form>
        </div>
...
const fetchFileData = async () => {
    ...
    try {
        ...
        fileId.value = parts[parts.length - 1];
        const token = getCookie(TOKEN_COOKIE_NAME);
        
        const response = await axios.get(`/api/public-files/${fileId.value}`, {
...
const handleSubmit = async () => {
  ...
  // Validation
  let hasError = false;
  if (!isEditMode.value && !form.file) { errors.file = 'File publik wajib diunggah'; hasError = true; }
  ...
  try {
    ...
    const endpoint = isEditMode.value ? `/api/public-files/${fileId.value}` : '/api/public-files';
    ...
    if (response.data.status === 'success') {
      await Swal.fire({
        icon: 'success',
        title: 'Berhasil!',
        text: isEditMode.value ? 'File publik berhasil diperbarui.' : 'File publik baru berhasil ditambahkan.',
    ...
    const message = error.response?.data?.message || 'Terjadi kesalahan saat menyimpan file publik.';
    await Swal.fire({
      icon: 'error',
      title: 'Oops...',
      text: message,
