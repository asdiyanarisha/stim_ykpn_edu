    ...
    <div class="flex-1 flex flex-col min-w-0 overflow-hidden">
      ...
      >
        <div v-if="!isAuthenticated || itemsLoading" class="absolute inset-0 z-[60] bg-slate-50/80 backdrop-blur-sm flex items-center justify-center">
            <div class="flex flex-col items-center">
                ...
                <p class="text-slate-500 font-medium animate-pulse">Memuat Data Jabatan...</p>
            </div>
        </div>
      </transition>
      ...
      <main v-if="isAuthenticated" class="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8">
        ...
        <div class="mb-8">
          ...
          <p class="text-slate-500">Kelola jabatan STIM YKPN.</p>
        </div>

        <!-- Toolbar Section -->
        <div class="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div class="w-full sm:max-w-md">
            <AppInput v-model="searchQuery" placeholder="Cari jabatan..." id="search-job-titles">
              <template #icon>
        ...
        <div class="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden flex flex-col">
            ...
            <table class="w-full text-left border-collapse min-w-[900px]">
              <thead>
                <tr class="bg-slate-50 border-b border-slate-100">
                  ...
                  <th class="px-6 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider w-2/5">Judul Jabatan</th>
                ...
                <tr v-for="item in paginatedItems" :key="item.id" class="hover:bg-slate-50 transition-colors group">
                  ...
                  <td class="px-6 py-4 text-right">
                    <div class="flex items-center justify-end gap-2">
                        <a :href="`/masterData/job-title/show/${item.id}`" class="inline-flex items-center gap-1 px-3 py-1.5 text-xs font-semibold text-indigo-600 bg-indigo-50 hover:bg-indigo-100 rounded-lg transition-colors">
                ...
                <tr v-if="paginatedItems.length === 0">
                  <td colspan="4" class="px-6 py-12 text-center text-slate-500 text-sm">
                    <div class="flex flex-col items-center">
                        ...
                        <p class="font-medium text-slate-600">Tidak ada data jabatan yang ditemukan.</p>
                        <p class="text-xs text-slate-400 mt-1">Gunakan kata kunci lain atau tambahkan jabatan baru.</p>
                    </div>
                  </td>
                </tr>
              </tbody>
          ...
          <div v-if="totalPages > 1" class="px-6 py-4 border-t border-slate-100 flex flex-col sm:flex-row items-center justify-between bg-slate-50/50 gap-4">
            <div class="text-sm text-slate-500 text-center sm:text-left">
              ...
              dari <span class="font-bold text-slate-900">{{ totalItems }}</span> jabatan
            </div>
        ...
        <div v-if="showFormModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm transition-opacity">
          <div class="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden transform transition-all translate-y-0 scale-100">
            <div class="p-6">
              ...
              <h3 class="text-xl font-bold text-slate-900 mb-2">{{ editingItem ? 'Ubah Jabatan' : 'Tambah Jabatan Baru' }}</h3>
              <p class="text-slate-500 text-sm mb-6">{{ editingItem ? 'Perbarui judul jabatan.' : 'Masukkan judul untuk jabatan baru.' }}</p>
              
              <form @submit.prevent="handleFormSubmit">
                <AppInput v-model="formTitle" label="Judul Jabatan" placeholder="Masukkan judul jabatan..." id="modal-job-title" :error="formError" />
              </form>
            </div>
            <div class="px-6 py-4 bg-slate-50 flex items-center justify-end gap-3 rounded-b-2xl">
              ...
              <button @click="handleFormSubmit" :disabled="isSubmitting" class="px-5 py-2 text-sm font-semibold text-white bg-indigo-600 hover:bg-indigo-700 active:scale-95 transition-all rounded-xl shadow-sm shadow-indigo-200 disabled:opacity-50 disabled:active:scale-100 flex items-center gap-2 cursor-pointer">
                ...
                <span v-if="!isSubmitting">{{ editingItem ? 'Simpan Perubahan' : 'Simpan Jabatan' }}</span>
        ...
        <div v-if="showDeleteModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm transition-opacity">
          <div class="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden transform transition-all translate-y-0 scale-100">
            <div class="p-6">
              ...
              <h3 class="text-xl font-bold text-slate-900 mb-2">Hapus {{ isBulkDelete ? selectedItems.length + ' Jabatan Terpilih' : 'Jabatan' }}?</h3>
              <p class="text-slate-500 text-sm">
                Tindakan ini tidak dapat dibatalkan. Data jabatan yang dihapus akan hilang dari sistem secara permanen.
              </p>
            </div>
            <div class="px-6 py-4 bg-slate-50 flex items-center justify-end gap-3 rounded-b-2xl">
...
const fetchJobTitles = async () => {
  ...
  try {
    ...
    Swal.fire({ icon: 'error', title: 'Kesalahan Sistem', text: 'Gagal mengambil data jabatan.', confirmButtonColor: '#4f46e5' });
  } finally {
...
const handleFormSubmit = async () => {
  if (!formTitle.value.trim()) {
    formError.value = 'Judul jabatan wajib diisi';
    return;
  }
  ...
  try {
    ...
    const endpoint = editingItem.value ? `/api/job-titles/${editingItem.value.id}` : '/api/job-titles';
    const response = await axios.post(endpoint, payload, { headers: { Authorization: `Bearer ${token}` } });
    if (response.data.status === 'success') {
      fetchJobTitles();
      closeFormModal();
      Swal.fire({ icon: 'success', title: 'Berhasil!', text: editingItem.value ? 'Jabatan berhasil diperbarui.' : 'Jabatan baru berhasil ditambahkan.', timer: 2000, showConfirmButton: false });
    }
  } catch (error) {
    const message = error.response?.data?.message || 'Terjadi kesalahan saat menyimpan jabatan.';
...
const executeDelete = async () => {
  ...
  try {
    ...
    if (isBulkDelete.value) {
      await axios.post('/api/job-titles/bulk-delete', { ids: selectedItems.value }, { headers: { Authorization: `Bearer ${token}` } });
      selectedItems.value = [];
    } else {
      await axios.delete(`/api/job-titles/${itemToDeleteId.value}`, { headers: { Authorization: `Bearer ${token}` } });
    }
    fetchJobTitles();
    closeDeleteModal();
    Swal.fire({ icon: 'success', title: 'Berhasil!', text: 'Data jabatan telah dihapus.', timer: 2000, showConfirmButton: false });
  } catch (error) {
    Swal.fire({ icon: 'error', title: 'Gagal!', text: 'Gagal menghapus jabatan.' });
  } finally {
    isDeleting.value = false;
  }
};
