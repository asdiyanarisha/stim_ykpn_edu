        ...
        >
          <div v-if="!isAuthenticated || isLoading" class="absolute inset-0 z-[60] bg-slate-50/80 backdrop-blur-sm flex items-center justify-center">
              <div class="flex flex-col items-center">
                  ...
                  <p class="text-slate-500 font-medium animate-pulse">Memuat Detail Brosur...</p>
              </div>
          </div>
        </transition>

        <div v-if="brochure.id" class="max-w-4xl mx-auto space-y-8 pb-12">
          ...
          <div class="bg-white rounded-[2rem] shadow-sm border border-slate-100 overflow-hidden">
            <div class="p-8 md:p-12">
                ...
                <div class="flex items-center gap-4 mb-8">
                    ...
                    <div>
                        <span class="px-3 py-1 bg-indigo-50 text-indigo-600 text-[10px] font-bold uppercase tracking-wider rounded-lg border border-indigo-100 inline-block mb-2">Dokumen Brosur</span>
                        <h1 class="text-3xl font-extrabold text-slate-900 leading-tight">{{ brochure.title || 'Tanpa Judul' }}</h1>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                    <div class="md:col-span-2 space-y-6">
                        <div class="space-y-3">
                            ...
                            <p class="text-slate-600 leading-relaxed whitespace-pre-wrap">{{ brochure.description || 'Tidak ada deskripsi tersedia.' }}</p>
                        </div>
                    </div>
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="bg-slate-50 rounded-2xl p-6 border border-slate-100 space-y-4">
                                <div class="flex flex-col gap-1">
                                    ...
                                    <span class="text-slate-700 font-semibold">{{ brochure.file_size || '-' }}</span>
                                </div>
                                <div class="flex flex-col gap-1">
                                    <span class="text-[10px] font-bold uppercase tracking-wider text-slate-400">Tanggal Unggah</span>
                                    <span class="text-slate-700 font-semibold">{{ formatDate(brochure.created_at) }}</span>
                                </div>
                                <div class="pt-2">
                                    <a v-if="brochure.file_path" :href="brochure.file_path" target="_blank" class="w-full inline-flex items-center justify-center gap-2 px-4 py-3 bg-indigo-600 text-white rounded-xl font-bold text-sm hover:bg-indigo-700 transition-all shadow-md shadow-indigo-100">
                ...
                <div class="mt-12 flex items-center gap-6 text-sm text-slate-400 border-t border-slate-50 pt-8">
                    <div class="flex items-center gap-2">
                        ...
                        Terakhir diubah: {{ formatDate(brochure.updated_at || brochure.created_at) }}
                    </div>
                </div>
            </div>
          </div>
          ...
          <div class="flex justify-between items-center bg-white p-6 rounded-[1.5rem] shadow-sm border border-slate-100">
            <div class="text-sm text-slate-400 italic">
                Informasi brosur ini digunakan untuk publikasi materi di website.
            </div>
            <div class="flex gap-3">
                <a :href="`/brochure/edit/${brochure.id}`">
...
const fetchBrochureDetail = async () => {
  ...
  try {
    ...
    const response = await axios.get(`/api/brochures/${id}`, {
      headers: { Authorization: `Bearer ${token}` }
    });
    
    if (response.data.status === 'success') {
      brochure.value = response.data.data;
    }
  } catch (error) {
    console.error('Error fetching brochure detail:', error);
