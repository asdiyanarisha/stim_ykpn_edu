        ...
        <div class="max-w-4xl mx-auto mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <div class="flex items-center gap-2 text-sm text-slate-500 mb-2">
              <a href="/student/ukm" class="hover:text-indigo-600 transition-colors">UKM</a>
              <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" /></svg>
              <span class="text-slate-900 font-medium">{{ isEditMode ? 'Ubah UKM' : 'Tambah UKM' }}</span>
            </div>
            <h1 class="text-2xl font-bold text-slate-900">{{ isEditMode ? 'Ubah Informasi UKM' : 'Tambah UKM Baru' }}</h1>
            <p class="text-slate-500">{{ isEditMode ? 'Perbarui informasi UKM.' : 'Tambahkan UKM baru ke dalam sistem.' }}</p>
          </div>
          ...
          <form @submit.prevent="handleSubmit" class="space-y-6">
            ...
            <div class="bg-white rounded-3xl p-8 shadow-sm border border-slate-100">
              ...
              <div class="flex flex-col items-center justify-center border-2 border-dashed border-slate-200 rounded-3xl p-8 hover:border-indigo-400 hover:bg-indigo-50/30 transition-all group relative overflow-hidden cursor-pointer" @click="$refs.fileInput.click()">
                ...
                <template v-if="!imagePreview">
                  ...
                  <p class="text-slate-600 font-bold">Pilih Logo UKM</p>
                ...
                <div v-else class="w-48 h-48 rounded-2xl overflow-hidden shadow-lg border border-slate-200 relative">
                  ...
                  <div class="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center backdrop-blur-sm">
                    <span class="px-5 py-2 bg-white text-slate-900 font-bold rounded-xl text-sm shadow-xl">Ganti Logo</span>
                  </div>
                </div>
              </div>
            ...
            <div class="bg-white rounded-3xl p-8 shadow-sm border border-slate-100">
              ...
              <div class="space-y-6">
                ...
                <div class="field-judul-besar">
                    <AppInput v-model="form.title" label="Nama UKM" placeholder="Masukkan nama UKM..." id="ukm-title" :error="errors.title" />
                </div>
                
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    <AppInput v-model="form.location" label="Lokasi" placeholder="Lokasi UKM..." id="ukm-location" :error="errors.location" />
                </div>

                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 mt-6">
                    <AppInput v-model="form.day" label="Hari" placeholder="Contoh: Senin, Rabu" id="ukm-day" :error="errors.day" />
                    <AppInput v-model="form.time" label="Waktu" placeholder="Contoh: 15:00 - 17:00" id="ukm-time" :error="errors.time" />
                </div>

                <!-- WYSIWYG Editor for Content -->
                <div class="space-y-1 mt-6">
                  <label class="block text-sm font-bold text-slate-700 mb-1.5 ml-1">Deskripsi UKM</label>
            ...
            <div class="bg-white rounded-3xl p-6 shadow-sm border border-slate-100 flex items-center justify-end gap-4">
              ...
              <AppButton variant="primary" size="md" type="submit" :disabled="isSubmitting">
                ...
                {{ isSubmitting ? (isEditMode ? 'Menyimpan...' : 'Menambahkan...') : (isEditMode ? 'Simpan Perubahan' : 'Simpan UKM') }}
              </AppButton>
            </div>
          </form>
        </div>
...
const editorOptions = {
  placeholder: 'Tulis deskripsi UKM di sini...',
  theme: 'snow',
  modules: {
    toolbar: [
...
const fetchItemData = async () => {
    ...
    try {
        ...
        const response = await axios.get(`/api/student-ukms/${itemId.value}`, {
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (form.day) formData.append('day', form.day);
    ...
    const endpoint = isEditMode.value ? `/api/student-ukms/${itemId.value}` : '/api/student-ukms';
    ...
    if (response.data.status === 'success') {
      await Swal.fire({
        icon: 'success',
        title: 'Berhasil!',
        text: isEditMode.value ? 'UKM berhasil diperbarui.' : 'UKM baru berhasil ditambahkan.',
