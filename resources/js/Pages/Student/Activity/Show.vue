        ...
        >
          <div v-if="!isAuthenticated || isLoading" class="absolute inset-0 z-[60] bg-slate-50/80 backdrop-blur-sm flex items-center justify-center">
              <div class="flex flex-col items-center">
                  ...
                  <p class="text-slate-500 font-medium animate-pulse">Memuat Detail Kegiatan...</p>
              </div>
          </div>
        </transition>

        <div v-if="activity.id" class="max-w-4xl mx-auto space-y-8 pb-12">
          <!-- Activity Detail Card -->
          <div class="bg-white rounded-[2rem] shadow-sm border border-slate-100 overflow-hidden">
            <!-- Header Image -->
            <div v-if="activity.header_image" class="w-full aspect-[21/9] overflow-hidden border-b border-slate-100">
                <img :src="activity.header_image" class="w-full h-full object-cover">
            </div>
            ...
            <div class="p-8 md:p-12">
                ...
                <div class="flex flex-wrap items-center gap-6 mb-8 text-sm text-slate-400">
                    <div class="flex items-center gap-2 text-indigo-600 font-bold bg-indigo-50 px-3 py-1 rounded-lg">
                        ...
                        {{ activity.location || 'Lokasi tidak disebutkan' }}
                    </div>
                    <div class="flex items-center gap-2">
                        ...
                        <div class="flex flex-col">
                            <span class="text-slate-400 text-[10px] uppercase tracking-wider font-bold">Waktu Kegiatan</span>
                            <span class="font-bold text-slate-800 text-sm">
                                {{ formatDate(activity.start_date) }} 
                                <template v-if="activity.end_date"> - {{ formatDate(activity.end_date) }}</template>
                            </span>
                        </div>
                    </div>
                    <div class="flex items-center gap-2">
                        <span v-if="activity.is_publish" class="px-3 py-1 bg-emerald-50 text-emerald-600 rounded-lg text-xs font-bold uppercase tracking-wider border border-emerald-100"> Published </span>
                        <span v-else class="px-3 py-1 bg-slate-100 text-slate-600 rounded-lg text-xs font-bold uppercase tracking-wider border border-slate-200"> Draft </span>
                    </div>
                </div>

                <div class="mb-10">
                    <h1 class="text-3xl md:text-4xl font-extrabold text-slate-900 leading-tight">{{ activity.title || 'Tanpa Judul' }}</h1>
                </div>
                ...
                <div class="prose prose-slate max-w-none mb-10">
                    ...
                    <div v-if="activity.description" v-html="activity.description" class="text-slate-600 leading-relaxed quill-content"></div>
                    <div v-else class="text-slate-400 italic">Tidak ada deskripsi untuk kegiatan ini.</div>
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div class="p-4 bg-slate-50 border border-slate-100 rounded-2xl">
                        ...
                        <p class="font-bold text-slate-700">{{ formatFullDate(activity.created_at) }}</p>
                    </div>
                    <div class="p-4 bg-slate-50 border border-slate-100 rounded-2xl">
                        <p class="text-[10px] uppercase tracking-widest font-bold text-slate-400 mb-1">Terakhir Diperbarui</p>
                        <p class="font-bold text-slate-700">{{ formatFullDate(activity.updated_at) }}</p>
                    </div>
                </div>
            </div>
          </div>
          ...
          <div class="flex justify-between items-center bg-white p-6 rounded-[1.5rem] shadow-sm border border-slate-100">
            <div class="text-sm text-slate-400 italic">
                Informasi detail kegiatan mahasiswa.
            </div>
            <div class="flex gap-3">
                <a :href="`/student/activity/edit/${activity.id}`">
...
const fetchActivityDetail = async () => {
  ...
  try {
    ...
    const response = await axios.get(`/api/student-activities/${id}`, {
