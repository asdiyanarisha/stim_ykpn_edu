  ...
  <div v-if="isAuthenticated" class="flex h-screen bg-slate-50 overflow-hidden">
    ...
    <div class="flex-1 flex flex-col min-w-0 overflow-hidden">
      ...
      >
        <div v-if="!isAuthenticated || itemsLoading" class="absolute inset-0 z-[60] bg-slate-50/80 backdrop-blur-sm flex items-center justify-center">
            <div class="flex flex-col items-center">
                ...
                <p class="text-slate-500 font-medium animate-pulse">Memuat Data Kegiatan...</p>
            </div>
        </div>
      </transition>
      ...
      <main v-if="isAuthenticated" class="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8">
        <!-- Header Section -->
        <div class="mb-8">
          <h1 class="text-2xl font-bold text-slate-900">Kegiatan Mahasiswa</h1>
          <p class="text-slate-500">Kelola informasi kegiatan mahasiswa STIM YKPN.</p>
        </div>

        <!-- Toolbar Section -->
        <div class="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div class="w-full sm:max-w-md">
            <AppInput v-model="searchQuery" placeholder="Cari judul kegiatan..." id="search-activities">
              <template #icon>
          ...
          <div class="flex flex-wrap items-center gap-3">
            ...
            <!-- Add Activity Button -->
            <a href="/student/activity/create">
            ...
            <table class="w-full text-left border-collapse min-w-[900px]">
              <thead>
                <tr class="bg-slate-50 border-b border-slate-100">
                  ...
                  <th class="px-6 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider w-2/5">Judul Kegiatan</th>
              ...
              <tbody class="divide-y divide-slate-100">
                <tr v-for="item in paginatedItems" :key="item.id" class="hover:bg-slate-50 transition-colors group">
                  ...
                  <td class="px-6 py-4">
                    <span class="text-sm text-slate-600">{{ item.start_date ? formatDate(item.start_date) : '-' }}</span>
                  </td>
                  <td class="px-6 py-4">
                    <span v-if="item.is_publish" class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-emerald-100 text-emerald-800">
                        Published
                    </span>
                    <span v-else class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-slate-100 text-slate-800">
                        Draft
                    </span>
                  </td>
                  <td class="px-6 py-4 text-right">
                    <div class="flex items-center justify-end gap-2">
                        <a :href="`/student/activity/show/${item.id}`" class="inline-flex items-center gap-1 px-3 py-1.5 text-xs font-semibold text-indigo-600 bg-indigo-50 hover:bg-indigo-100 rounded-lg transition-colors">
                            <svg xmlns="http://www.w3.org/2000/svg" class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg>
                            Detail
                        </a>
                        <a :href="`/student/activity/edit/${item.id}`" class="inline-flex items-center gap-1 px-3 py-1.5 text-xs font-semibold text-amber-600 bg-amber-50 hover:bg-amber-100 rounded-lg transition-colors">
                ...
                <tr v-if="paginatedItems.length === 0">
                  <td colspan="6" class="px-6 py-12 text-center text-slate-500 text-sm">
                    <div class="flex flex-col items-center">
                        ...
                        <p class="font-medium text-slate-600">Tidak ada data kegiatan yang ditemukan.</p>
                        <p class="text-xs text-slate-400 mt-1">Gunakan kata kunci lain atau tambahkan kegiatan baru.</p>
                    </div>
                  </td>
                </tr>
              </tbody>
        ...
        <div v-if="showDeleteModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm transition-opacity">
          <div class="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden transform transition-all translate-y-0 scale-100">
            <div class="p-6">
              ...
              <h3 class="text-xl font-bold text-slate-900 mb-2">Hapus {{ isBulkDelete ? selectedItems.length + ' Kegiatan Terpilih' : 'Kegiatan' }}?</h3>
              <p class="text-slate-500 text-sm">
                Tindakan ini tidak dapat dibatalkan. Data kegiatan yang dihapus akan hilang dari sistem secara permanen.
              </p>
            </div>
...
const fetchActivities = async () => {
  ...
  try {
    ...
    Swal.fire({
      ...
      text: 'Gagal mengambil data kegiatan mahasiswa.',
...
const executeDelete = async () => {
  ...
  try {
    ...
    if (isBulkDelete.value) {
      await axios.post('/api/student-activities/bulk-delete', { ids: selectedItems.value }, {
        headers: { Authorization: `Bearer ${token}` }
      });
      selectedItems.value = [];
    } else {
      await axios.delete(`/api/student-activities/${itemToDeleteId.value}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
    }
    fetchActivities();
    ...
    Swal.fire({ icon: 'error', title: 'Gagal!', text: 'Gagal menghapus kegiatan.' });
  } finally {
    isDeleting.value = false;
  }
};
