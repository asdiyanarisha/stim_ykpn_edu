<template>
  <div v-if="isAuthenticated" class="flex h-screen bg-slate-50 overflow-hidden">
    ...
    <div class="flex-1 flex flex-col min-w-0 overflow-hidden">
      ...
      >
        <div v-if="!isAuthenticated || videosLoading" class="absolute inset-0 z-[60] bg-slate-50/80 backdrop-blur-sm flex items-center justify-center">
            <div class="flex flex-col items-center">
                <div class="w-12 h-12 border-4 border-indigo-200 border-t-indigo-600 rounded-full animate-spin mb-4"></div>
                <p class="text-slate-500 font-medium animate-pulse">Memuat Data Video...</p>
            </div>
        </div>
      </transition>
      ...
      <main v-if="isAuthenticated" class="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8">
        <!-- Header Section -->
        <div class="mb-8">
          <h1 class="text-2xl font-bold text-slate-900">Manajemen Video</h1>
          <p class="text-slate-500">Kelola konten video Campus Life STIM YKPN.</p>
        </div>

        <!-- Toolbar Section -->
        <div class="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div class="w-full sm:max-w-md">
            <AppInput v-model="searchQuery" placeholder="Cari judul video..." id="search-videos">
              <template #icon>
          ...
          <div class="flex flex-wrap items-center gap-3">
            ...
            >
              <div v-if="selectedVideos.length > 0" class="flex items-center gap-3 z-10">
            ...
            <!-- Add Video Button -->
            <a href="/campus-life/video/create">
        ...
        <div class="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden flex flex-col">
              ...
              <thead>
                <tr class="bg-slate-50 border-b border-slate-100">
                  <th class="px-6 py-4 w-12">
                    <input type="checkbox" @change="toggleSelectAll" :checked="selectedVideos.length === paginatedVideos.length && paginatedVideos.length > 0" class="w-4 h-4 rounded border-slate-300 text-indigo-600 focus:ring-indigo-500 cursor-pointer transition-colors">
                  </th>
                  <th class="px-6 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider">Visual</th>
                  <th class="px-6 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider">Judul Video</th>
                  <th class="px-6 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider">Link</th>
                  <th class="px-6 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider text-right">Aksi</th>
                </tr>
              </thead>
              <tbody class="divide-y divide-slate-100">
                <tr v-for="item in paginatedVideos" :key="item.id" class="hover:bg-slate-50 transition-colors group">
                  <td class="px-6 py-4">
                    <input type="checkbox" :value="item.id" v-model="selectedVideos" class="w-4 h-4 rounded border-slate-300 text-indigo-600 focus:ring-indigo-500 cursor-pointer transition-colors">
                  </td>
                  <td class="px-6 py-4">
                    <div class="w-20 h-12 rounded-lg border border-slate-200 overflow-hidden bg-slate-100">
                      <img v-if="getYoutubeThumbnail(item.link)" :src="getYoutubeThumbnail(item.link)" class="w-full h-full object-cover" @error="(e) => e.target.src = '/placeholder-video.png'">
                  ...
                  <td class="px-6 py-4 text-right">
                    <div class="flex items-center justify-end gap-2">
                        <a :href="`/campus-life/video/show/${item.id}`" class="inline-flex items-center gap-1 px-3 py-1.5 text-xs font-semibold text-indigo-600 bg-indigo-50 hover:bg-indigo-100 rounded-lg transition-colors">
                            <svg xmlns="http://www.w3.org/2000/svg" class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg>
                            Detail
                        </a>
                        <a :href="`/campus-life/video/edit/${item.id}`" class="inline-flex items-center gap-1 px-3 py-1.5 text-xs font-semibold text-amber-600 bg-amber-50 hover:bg-amber-100 rounded-lg transition-colors">
                ...
                <tr v-if="paginatedVideos.length === 0">
                  <td colspan="5" class="px-6 py-12 text-center text-slate-500 text-sm">
                    <div class="flex flex-col items-center">
                        ...
                        <p class="font-medium text-slate-600">Tidak ada data video yang ditemukan.</p>
                        <p class="text-xs text-slate-400 mt-1">Gunakan kata kunci lain atau tambahkan video baru.</p>
                    </div>
                  </td>
                </tr>
              </tbody>
          ...
          <div v-if="totalPages > 1" class="px-6 py-4 border-t border-slate-100 flex flex-col sm:flex-row items-center justify-between bg-slate-50/50 gap-4">
            <div class="text-sm text-slate-500 text-center sm:text-left">
              ...
              dari <span class="font-bold text-slate-900">{{ totalItems }}</span> video
            </div>
        ...
        <div v-if="showDeleteModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm transition-opacity">
          <div class="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden transform transition-all translate-y-0 scale-100">
            <div class="p-6">
              ...
              <h3 class="text-xl font-bold text-slate-900 mb-2">Hapus {{ isBulkDelete ? selectedVideos.length + ' Video Terpilih' : 'Video' }}?</h3>
              <p class="text-slate-500 text-sm">
                Tindakan ini tidak dapat dibatalkan. Video yang dihapus akan hilang dari sistem secara permanen.
              </p>
            </div>
...
const fetchVideos = async () => {
  ...
  try {
    ...
    const response = await axios.get('/api/campus-live-videos', {
...
const limit = ref(10);
const totalItems = computed(() => filteredVideos.value.length);
const totalPages = computed(() => Math.ceil(totalItems.value / limit.value) || 1);

const paginatedVideos = computed(() => {
  const offset = (currentPage.value - 1) * limit.value;
  return filteredVideos.value.slice(offset, offset + limit.value);
});
...
const toggleSelectAll = (e) => {
  if (e.target.checked) selectedVideos.value = paginatedVideos.value.map(v => v.id);
  else selectedVideos.value = [];
};
...
const executeDelete = async () => {
  ...
  try {
    ...
    if (isBulkDelete.value) {
      await axios.post('/api/campus-live-videos/bulk-delete', { ids: selectedVideos.value }, {
        headers: { Authorization: `Bearer ${token}` }
      });
      selectedVideos.value = [];
    } else {
      await axios.delete(`/api/campus-live-videos/${itemToDeleteId.value}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
    }
    fetchVideos();
    ...
    Swal.fire({
      icon: 'success',
      title: 'Berhasil!',
      text: 'Data video telah dihapus.',
      timer: 2000,
      showConfirmButton: false
    });
  } catch (error) {
    console.error('Error deleting video:', error);
    Swal.fire({ icon: 'error', title: 'Gagal!', text: 'Gagal menghapus video.' });
  } finally {
    isDeleting.value = false;
  }
};
...
const getYoutubeThumbnail = (url) => {
    if (!url) return null;
    const videoId = url.match(/(?:https?:\/\/(?:www\.)?youtube\.com\/watch\?v=|https?:\/\/youtu\.be\/|https?:\/\/(?:www\.)?youtube\.com\/embed\/)([^&?]+)/);
