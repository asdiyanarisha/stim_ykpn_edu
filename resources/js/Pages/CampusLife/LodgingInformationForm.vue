        ...
        <div class="max-w-4xl mx-auto mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <div class="flex items-center gap-2 text-sm text-slate-500 mb-2">
              <span class="hover:text-indigo-600 transition-colors">Campus Life</span>
              <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" /></svg>
              <span class="text-slate-900 font-medium">Informasi Tempat Tinggal</span>
            </div>
            <h1 class="text-2xl font-bold text-slate-900">Informasi Tempat Tinggal</h1>
            <p class="text-slate-500">Kelola informasi mengenai tempat tinggal bagi mahasiswa STIM YKPN.</p>
          </div>
        </div>
          ...
          <form @submit.prevent="handleSubmit" class="space-y-6">
            ...
            <div class="bg-white rounded-3xl p-8 shadow-sm border border-slate-100">
              ...
              <div class="flex flex-col items-center justify-center border-2 border-dashed border-slate-200 rounded-3xl p-12 hover:border-indigo-400 hover:bg-indigo-50/30 transition-all group relative overflow-hidden cursor-pointer" @click="$refs.fileInput.click()">
                ...
                <template v-if="!imagePreview">
                  ...
                  <p class="text-slate-400 text-sm mt-1">Saran: Gambar lingkungan tempat tinggal (Maks. 5MB)</p>
                </template>
            ...
            <div class="bg-white rounded-3xl p-8 shadow-sm border border-slate-100">
              <h2 class="text-lg font-bold text-slate-800 mb-6 flex items-center gap-2">
                ...
                Detail Informasi Tempat Tinggal
              </h2>

              <div class="space-y-6">
                <AppInput v-model="form.title" label="Judul" placeholder="Misal: Informasi Kost & Asrama Mahasiswa" id="lodging-title" :error="errors.title" />
...
const editorOptions = {
  placeholder: 'Tuliskan informasi tempat tinggal mahasiswa di sini...',
  theme: 'snow',
  modules: {
    toolbar: [
...
const fetchData = async () => {
    ...
    try {
        ...
        const response = await axios.get('/api/lodging-information', {
        ...
        Swal.fire({ icon: 'error', title: 'Kesalahan', text: 'Gagal memuat data informasi tempat tinggal.' });
    } finally {
...
const handleSubmit = async () => {
  ...
  try {
    ...
    const response = await axios.post('/api/lodging-information', formData, {
      headers: {
