      ...
      >
        <div v-if="isLoading" class="absolute inset-0 z-[60] bg-slate-50/80 backdrop-blur-sm flex items-center justify-center">
            <div class="flex flex-col items-center">
                ...
                <p class="text-slate-500 font-medium animate-pulse">Menyiapkan Formulir...</p>
            </div>
        </div>
      </transition>
        ...
        <div class="max-w-4xl mx-auto mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <div class="flex items-center gap-2 text-sm text-slate-500 mb-2">
              ...
              <span class="text-slate-900 font-medium">Edit Banner</span>
            </div>
            <h1 class="text-2xl font-bold text-slate-900">Ubah Banner</h1>
            <p class="text-slate-500">Sesuaikan informasi banner untuk memperbarui tampilan di halaman depan.</p>
          </div>
...
const fetchBannerData = async () => {
  try {
    ...
    if (response.data.status === 'success') {
      ...
      form.button_text = data.button_text || '';
      form.link = data.link || '';
...
const handleSubmit = async () => {
  ...
  try {
    ...
    // In edit, ONLY append image if a NEW one was selected
    if (form.url_image instanceof File) {
      formData.append('image', form.url_image);
    }

    // Use POST with ID for update to handle Multipart/Form-Data properly
    const response = await axios.post(`/api/banners/${form.id}`, formData, {
      headers: {
    ...
    const message = error.response?.data?.message || 'Terjadi kesalahan saat memperbarui banner.';
    await Swal.fire({
      icon: 'error',
      title: 'Oops...',
      text: message,
