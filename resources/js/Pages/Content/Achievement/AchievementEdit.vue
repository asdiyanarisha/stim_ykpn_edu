<template>
  <div v-if="isAuthenticated" class="flex h-screen bg-slate-50 overflow-hidden font-sans">
    ...
    <div class="flex-1 flex flex-col min-w-0 overflow-hidden">
      ...
      >
        <div v-if="isLoading || isSubmitting" class="absolute inset-0 z-[60] bg-slate-50/80 backdrop-blur-sm flex items-center justify-center">
            <div class="flex flex-col items-center">
                <div class="w-12 h-12 border-4 border-indigo-200 border-t-indigo-600 rounded-full animate-spin mb-4"></div>
                <p class="text-slate-500 font-medium animate-pulse">{{ isLoading ? 'Mengambil data...' : 'Menyimpan perubahan...' }}</p>
            </div>
        </div>
      </transition>
        ...
        <div class="max-w-4xl mx-auto mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <div class="flex items-center gap-2 text-sm text-slate-500 mb-2">
              <a href="/content/profil/achievement" class="hover:text-indigo-600 transition-colors">Manajemen Prestasi</a>
              <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" /></svg>
              <span class="text-slate-900 font-medium">Ubah Prestasi</span>
            </div>
            <h1 class="text-2xl font-bold text-slate-900">Ubah Informasi Prestasi</h1>
            <p class="text-slate-500">Perbarui detail pencapaian STIM YKPN.</p>
          </div>
          ...
          <form @submit.prevent="handleSubmit" class="space-y-6">
            ...
            <div class="bg-white rounded-3xl p-8 shadow-sm border border-slate-100">
              <h2 class="text-lg font-bold text-slate-800 mb-6 flex items-center gap-2">
                ...
                Visual Utama Prestasi
              </h2>

              <div class="flex flex-col items-center justify-center border-2 border-dashed border-slate-200 rounded-3xl p-12 hover:border-indigo-400 hover:bg-indigo-50/30 transition-all group relative overflow-hidden cursor-pointer aspect-video" @click="$refs.fileInput.click()">
                ...
                <template v-if="!imagePreview">
                  ...
                  <p class="text-slate-400 text-sm mt-1">Ukuran Maks. 5MB (JPG, PNG, WEBP)</p>
                </template>

                <div v-if="imagePreview" class="w-full h-full rounded-2xl overflow-hidden shadow-lg border border-slate-200 relative">
            ...
            <div class="bg-white rounded-3xl p-8 shadow-sm border border-slate-100">
              ...
              <div class="space-y-6">
                <AppInput v-model="form.title" label="Judul Prestasi" placeholder="Misal: Juara 1 Lomba Akuntansi Nasional..." id="achievement-title" :error="errors.title" />

                <!-- Quill Editor for Content -->
                <div class="space-y-1">
                  <label class="block text-sm font-bold text-slate-700 mb-1.5 ml-1">Deskripsi Lengkap</label>
...
const editorOptions = {
  placeholder: 'Tuliskan detail prestasi di sini...',
  theme: 'snow',
  modules: {
    toolbar: [
...
const goBack = () => {
    window.location.href = '/content/profil/achievement';
};
...
const fetchAchievement = async (id) => {
    ...
    try {
        ...
        Swal.fire({ icon: 'error', title: 'Gagal', text: 'Gagal mengambil data prestasi.' }).then(() => {
            goBack();
        });
    } finally {
...
const handleSubmit = async () => {
    ...
    if (!form.title) { errors.title = 'Judul prestasi wajib diisi'; hasError = true; }
    if (!form.content || form.content === '<p><br></p>') { errors.content = 'Deskripsi prestasi wajib diisi'; hasError = true; }
    ...
    try {
        ...
        const response = await axios.post(`/api/achievements/${achievementId.value}`, formData, {
...
onMounted(async () => {
  ...
  try {
    ...
    achievementId.value = parts[parts.length - 1];
    if (achievementId.value) fetchAchievement(achievementId.value);
  } catch (error) {
