        ...
        <div class="max-w-4xl mx-auto mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <div class="flex items-center gap-2 text-sm text-slate-500 mb-2">
              ...
              <span class="text-slate-900 font-medium">Sambutan Ketua</span>
            </div>
            <h1 class="text-2xl font-bold text-slate-900">Sambutan Ketua STIM YKPN</h1>
            <p class="text-slate-500">Kelola pesan dan foto sambutan ketua untuk ditampilkan di website.</p>
          </div>
        </div>
        ...
        <div class="max-w-4xl mx-auto">
          <form @submit.prevent="handleSubmit" class="space-y-6">
            ...
            <div class="bg-white rounded-3xl p-8 shadow-sm border border-slate-100">
              ...
              <div class="flex flex-col items-center justify-center border-2 border-dashed border-slate-200 rounded-3xl p-12 hover:border-indigo-400 hover:bg-indigo-50/30 transition-all group relative overflow-hidden cursor-pointer" @click="$refs.fileInput.click()">
                ...
                <template v-if="!imagePreview">
                  ...
                  <p class="text-slate-400 text-sm mt-1">Saran: Foto formal ketua (Maks. 5MB)</p>
                </template>

                <div v-if="imagePreview" class="w-full max-w-sm aspect-[3/4] mx-auto rounded-2xl overflow-hidden shadow-lg border border-slate-200 relative">
            ...
            <div class="bg-white rounded-3xl p-8 shadow-sm border border-slate-100">
              ...
              <div class="space-y-6">
                <AppInput v-model="form.title" label="Judul" placeholder="Misal: Sambutan Ketua STIM YKPN" id="greeting-title" :error="errors.title" />

                <!-- Quill Editor for Content -->
                <div class="space-y-1">
                  <label class="block text-sm font-bold text-slate-700 mb-1.5 ml-1">Isi Sambutan</label>
...
const fetchData = async () => {
    ...
    try {
        ...
        const response = await axios.get('/api/greeting-chief', {
        ...
        Swal.fire({ icon: 'error', title: 'Kesalahan', text: 'Gagal memuat data sambutan.' });
    } finally {
...
const handleSubmit = async () => {
  ...
  try {
    ...
    const response = await axios.post('/api/greeting-chief', formData, {
      headers: {
