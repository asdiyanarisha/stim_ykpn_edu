  ...
  <div v-if="isAuthenticated" class="flex h-screen bg-slate-50 overflow-hidden">
    ...
    <div class="flex-1 flex flex-col min-w-0 overflow-hidden">
      ...
      >
        <div v-if="!isAuthenticated || itemsLoading" class="absolute inset-0 z-[60] bg-slate-50/80 backdrop-blur-sm flex items-center justify-center">
            <div class="flex flex-col items-center">
                ...
                <p class="text-slate-500 font-medium animate-pulse">Memuat Data Kategori Kalender...</p>
            </div>
        </div>
      </transition>
      ...
      <main v-if="isAuthenticated" class="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8">
        <!-- Header Section -->
        <div class="mb-8">
          <h1 class="text-2xl font-bold text-slate-900">Kategori Kalender Akademik</h1>
          <p class="text-slate-500">Kelola kategori acara/kegiatan kalender akademik STIM YKPN.</p>
        </div>

        <!-- Toolbar Section -->
        <div class="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div class="w-full sm:max-w-md">
            <AppInput v-model="searchQuery" placeholder="Cari kategori kalender..." id="search-academic-calendar-categories">
              <template #icon>
            ...
            <table class="w-full text-left border-collapse min-w-[900px]">
              <thead>
                <tr class="bg-slate-50 border-b border-slate-100">
                  ...
                  <th class="px-6 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider w-1/3">Nama Kategori</th>
                  <th class="px-6 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider w-1/5 text-center">Ikon</th>
                  <th class="px-6 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider w-1/5 text-center">Warna Label</th>
                ...
                <tr v-for="item in paginatedItems" :key="item.id" class="hover:bg-slate-50 transition-colors group">
                  ...
                  <td class="px-6 py-4">
                    <div class="flex flex-col">
                        <span class="font-semibold text-slate-900 text-sm group-hover:text-indigo-600 transition-colors">{{ item.nama || 'Tanpa Nama' }}</span>
                        <span class="text-xs text-slate-400 mt-0.5">Dibuat: {{ formatDate(item.created_at) }}</span>
                    </div>
                  </td>
                  <td class="px-6 py-4 text-center">
                    <div class="inline-flex items-center justify-center w-10 h-10 rounded-xl bg-slate-100 text-slate-700 text-lg">
                      <i :class="['bi', item.ikon || 'bi-calendar']"></i>
                    </div>
                    <span class="block text-[10px] text-slate-400 mt-1 font-mono">{{ item.ikon || 'bi-calendar' }}</span>
                  </td>
                  <td class="px-6 py-4 text-center">
                    <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-semibold" :style="{ backgroundColor: item.warna + '20', color: item.warna }">
                      <span class="w-2.5 h-2.5 rounded-full" :style="{ backgroundColor: item.warna }"></span>
                      {{ item.warna || '#3B82F6' }}
                    </div>
                  </td>
                  <td class="px-6 py-4 text-right">
                    <div class="flex items-center justify-end gap-2">
                        <button @click="openFormModal(item)" class="inline-flex items-center gap-1 px-3 py-1.5 text-xs font-semibold text-amber-600 bg-amber-50 hover:bg-amber-100 rounded-lg transition-colors cursor-pointer border-none">
                        ...
                        <button @click="openDeleteModal(item.id)" class="inline-flex items-center gap-1 px-3 py-1.5 text-xs font-semibold text-rose-600 bg-rose-50 hover:bg-rose-100 rounded-lg transition-colors cursor-pointer border-none">
                ...
                <tr v-if="paginatedItems.length === 0">
                  <td colspan="5" class="px-6 py-12 text-center text-slate-500 text-sm">
                    <div class="flex flex-col items-center">
                        ...
                        <p class="font-medium text-slate-600">Tidak ada data kategori kalender yang ditemukan.</p>
        ...
        <div v-if="showFormModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm transition-opacity">
          <div class="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden transform transition-all translate-y-0 scale-100">
            <div class="p-6">
              ...
              <p class="text-slate-500 text-sm mb-6">{{ editingItem ? 'Perbarui informasi kategori kalender.' : 'Masukkan informasi kategori kalender baru.' }}</p>
              
              <form @submit.prevent="handleFormSubmit" class="space-y-4">
                <AppInput v-model="formNama" label="Nama Kategori" placeholder="contoh: AKADEMIK, UJIAN..." id="modal-category-nama" :error="formErrors.nama" />
                
                <div class="space-y-1.5">
                  <label class="block text-sm font-medium text-slate-700">Pilih Ikon Preset</label>
                  <div class="grid grid-cols-8 gap-2 p-3 bg-slate-50 rounded-xl border border-slate-200">
                    <button 
                      ...
                      @click="formIkon = icon"
                      :class="[
                        'w-8 h-8 rounded-lg flex items-center justify-center text-lg border transition-all cursor-pointer',
                        formIkon === icon 
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div class="space-y-1">
                    <label class="block text-sm font-medium text-slate-700">Warna Label</label>
                    <div class="flex items-center gap-3">
                      <input type="color" v-model="formWarna" class="w-10 h-10 p-0.5 rounded-lg border border-slate-200 cursor-pointer bg-white" />
                      <input type="text" v-model="formWarna" class="flex-1 min-w-0 border rounded-xl border-slate-200 focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm py-2 px-3 bg-white" placeholder="#3B82F6" />
                    </div>
                  </div>

                  <AppInput v-model="formIkon" label="Atau Ketik Ikon Custom" placeholder="contoh: bi-calendar" id="modal-category-ikon" :error="formErrors.ikon" />
                </div>

                <div class="p-3 bg-slate-50 rounded-xl flex items-center gap-4 text-sm text-slate-600">
                  <div><strong>Preview Tampilan:</strong></div>
                  <div class="flex items-center gap-2 px-3 py-1 rounded-full text-xs font-semibold" :style="{ backgroundColor: formWarna + '20', color: formWarna }">
                    <i :class="['bi', formIkon || 'bi-calendar']" class="text-sm"></i>
                    {{ formNama || 'NAMA KATEGORI' }}
                  </div>
                </div>
              </form>
            </div>
            <div class="px-6 py-4 bg-slate-50 flex items-center justify-end gap-3 rounded-b-2xl">
              <button @click="closeFormModal" :disabled="isSubmitting" class="px-4 py-2 text-sm font-semibold text-slate-600 hover:text-slate-800 transition-colors cursor-pointer disabled:opacity-50 border-none bg-transparent">
                Batal
              </button>
              <button @click="handleFormSubmit" :disabled="isSubmitting" class="px-5 py-2 text-sm font-semibold text-white bg-indigo-600 hover:bg-indigo-700 active:scale-95 transition-all rounded-xl shadow-sm shadow-indigo-200 disabled:opacity-50 disabled:active:scale-100 flex items-center gap-2 cursor-pointer border-none">
          ...
          <div class="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden transform transition-all translate-y-0 scale-100">
            <div class="p-6">
              ...
              <p class="text-slate-500 text-sm">
                Tindakan ini tidak dapat dibatalkan. Data kategori kalender akademik yang dihapus akan hilang dari sistem secara permanen.
              </p>
            </div>
            <div class="px-6 py-4 bg-slate-50 flex items-center justify-end gap-3 rounded-b-2xl">
              <button @click="closeDeleteModal" :disabled="isDeleting" class="px-4 py-2 text-sm font-semibold text-slate-600 hover:text-slate-800 transition-colors cursor-pointer disabled:opacity-50 border-none bg-transparent">
                Batal
              </button>
              <button @click="executeDelete" :disabled="isDeleting" class="px-5 py-2 text-sm font-semibold text-white bg-rose-600 hover:bg-rose-700 active:scale-95 transition-all rounded-xl shadow-sm shadow-rose-200 disabled:opacity-50 disabled:active:scale-100 flex items-center gap-2 cursor-pointer border-none">
...
const fetchCategories = async () => {
  ...
  try {
    ...
    const response = await axios.get('/api/academic-calendar-categories', {
    ...
    Swal.fire({ icon: 'error', title: 'Kesalahan Sistem', text: 'Gagal mengambil data kategori kalender akademik.', confirmButtonColor: '#4f46e5' });
  } finally {
...
const filteredItems = computed(() => {
  ...
  return allItems.value.filter(n => (n.nama && n.nama.toLowerCase().includes(q)) || (n.ikon && n.ikon.toLowerCase().includes(q)) || (n.warna && n.warna.toLowerCase().includes(q)));
});
...
const formNama = ref('');
const formWarna = ref('#3B82F6');
const formIkon = ref('bi-calendar');
...
const openFormModal = (item = null) => {
  ...
  formNama.value = item ? (item.nama || '') : '';
  formWarna.value = item ? (item.warna || '#3B82F6') : '#3B82F6';
  formIkon.value = item ? (item.ikon || 'bi-calendar') : 'bi-calendar';
  formErrors.value = {};
...
const closeFormModal = () => {
  ...
  formNama.value = '';
  formWarna.value = '#3B82F6';
  formIkon.value = 'bi-calendar';
  formErrors.value = {};
};
...
const handleFormSubmit = async () => {
  ...
  if (!formNama.value.trim()) {
    formErrors.value.nama = 'Nama kategori wajib diisi';
    return;
  }
  ...
  try {
    ...
    const endpoint = editingItem.value ? `/api/academic-calendar-categories/${editingItem.value.id}` : '/api/academic-calendar-categories';
...
const executeDelete = async () => {
  ...
  try {
    ...
    if (isBulkDelete.value) {
      await axios.post('/api/academic-calendar-categories/bulk-delete', { ids: selectedItems.value }, { headers: { Authorization: `Bearer ${token}` } });
      selectedItems.value = [];
    } else {
      await axios.delete(`/api/academic-calendar-categories/${itemToDeleteId.value}`, { headers: { Authorization: `Bearer ${token}` } });
    }
    fetchCategories();
    closeDeleteModal();
    Swal.fire({ icon: 'success', title: 'Berhasil!', text: 'Data kategori kalender akademik telah dihapus.', timer: 2000, showConfirmButton: false });
  } catch (error) {
    Swal.fire({ icon: 'error', title: 'Gagal!', text: 'Gagal menghapus kategori kalender akademik.' });
  } finally {
    isDeleting.value = false;
  }
};
