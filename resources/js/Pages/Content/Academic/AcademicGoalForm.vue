      ...
      >
        <div v-if="isLoading" class="absolute inset-0 z-[60] bg-slate-50/80 backdrop-blur-sm flex items-center justify-center">
            <div class="flex flex-col items-center">
                ...
                <p class="text-slate-500 font-medium animate-pulse">Memuat Sasaran Pendidikan...</p>
            </div>
        </div>
      </transition>
        ...
        <div class="max-w-4xl mx-auto mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <div class="flex items-center gap-2 text-sm text-slate-500 mb-2">
              ...
              <span class="text-slate-900 font-medium">Sasaran Pendidikan</span>
            </div>
            <h1 class="text-2xl font-bold text-slate-900">Sasaran Pendidikan STIM YKPN</h1>
            <p class="text-slate-500">Kelola informasi mengenai sasaran dan orientasi pendidikan institusi.</p>
          </div>
        </div>
          ...
          <form @submit.prevent="handleSubmit" class="space-y-6">
            ...
            <div class="bg-white rounded-3xl p-8 shadow-sm border border-slate-100">
              ...
              <div class="flex flex-col items-center justify-center border-2 border-dashed border-slate-200 rounded-3xl p-12 hover:border-indigo-400 hover:bg-indigo-50/30 transition-all group relative overflow-hidden cursor-pointer" @click="$refs.fileInput.click()">
                ...
                <template v-if="!imagePreview">
                  ...
                  <p class="text-slate-400 text-sm mt-1">Saran: Gambar visual sasaran pendidikan (Maks. 5MB)</p>
                </template>
            ...
            <div class="bg-white rounded-3xl p-8 shadow-sm border border-slate-100">
              <h2 class="text-lg font-bold text-slate-800 mb-6 flex items-center gap-2">
                ...
                Detail Sasaran Pendidikan
              </h2>

              <div class="space-y-6">
                <AppInput v-model="form.title" label="Judul" placeholder="Misal: Sasaran Pendidikan STIM YKPN" id="goal-title" :error="errors.title" />
...
const editorOptions = {
  placeholder: 'Tuliskan rincian sasaran pendidikan di sini...',
  theme: 'snow',
  modules: {
    toolbar: [
...
const fetchData = async () => {
    ...
    try {
        ...
        const response = await axios.get('/api/academic-goals', {
        ...
        Swal.fire({ icon: 'error', title: 'Kesalahan', text: 'Gagal memuat data sasaran pendidikan.' });
    } finally {
...
const handleSubmit = async () => {
  ...
  try {
    ...
    const response = await axios.post('/api/academic-goals', formData, {
      headers: {
