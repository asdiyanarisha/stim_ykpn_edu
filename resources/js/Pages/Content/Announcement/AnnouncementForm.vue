        ...
        <div class="max-w-4xl mx-auto mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <div class="flex items-center gap-2 text-sm text-slate-500 mb-2">
              <a href="/announcement" class="hover:text-indigo-600 transition-colors">Manajemen Pengumuman</a>
              <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" /></svg>
              <span class="text-slate-900 font-medium">{{ isEditMode ? 'Ubah Pengumuman' : 'Tambah Pengumuman' }}</span>
            </div>
            <h1 class="text-2xl font-bold text-slate-900">{{ isEditMode ? 'Ubah Informasi Pengumuman' : 'Formulir Pengumuman Baru' }}</h1>
            <p class="text-slate-500">{{ isEditMode ? 'Perbarui konten pengumuman Anda untuk memberikan informasi terkini.' : 'Tulis dan publikasikan pengumuman terbaru untuk STIM YKPN.' }}</p>
          </div>
        ...
        <div class="max-w-4xl mx-auto">
          <form @submit.prevent="handleSubmit" class="space-y-6">
            ...
            <div class="bg-white rounded-3xl p-8 shadow-sm border border-slate-100">
              ...
              <div class="space-y-6">
                <AppInput v-model="form.title" label="Judul Pengumuman" placeholder="Masukkan judul pengumuman yang menarik..." id="announcement-title" :error="errors.title" />

                <!-- WYSIWYG Editor for Content -->
                <div class="space-y-1">
                  <label class="block text-sm font-bold text-slate-700 mb-1.5 ml-1">Isi Pengumuman</label>
                ...
                <div class="space-y-1">
                  ...
                  <div class="flex items-center gap-4">
                    ...
                    >
                      <input type="radio" v-model="form.status" :value="option.value" class="text-indigo-600 focus:ring-indigo-500 cursor-pointer">
                      <div>
                        <span class="text-sm font-semibold" :class="form.status === option.value ? 'text-indigo-700' : 'text-slate-700'">{{ option.label }}</span>
                        <p class="text-xs" :class="form.status === option.value ? 'text-indigo-500' : 'text-slate-400'">{{ option.desc }}</p>
                      </div>
                    </label>
                  </div>
                </div>
            ...
            <div class="bg-white rounded-3xl p-6 shadow-sm border border-slate-100 flex items-center justify-end gap-4">
              ...
              <AppButton variant="primary" size="md" type="submit" :disabled="isSubmitting">
                ...
                {{ isSubmitting ? (isEditMode ? 'Menyimpan...' : 'Menambahkan...') : (isEditMode ? 'Simpan Perubahan' : 'Simpan Pengumuman') }}
              </AppButton>
            </div>
          </form>
        </div>
...
const statusOptions = [
  { value: 'draft', label: 'Draft', desc: 'Simpan sebagai draf' },
  { value: 'published', label: 'Publikasi', desc: 'Langsung tampilkan' }
];
...
const fetchAnnouncementData = async () => {
    ...
    try {
        ...
        announcementId.value = parts[parts.length - 1];
        const token = getCookie(TOKEN_COOKIE_NAME);
        
        const response = await axios.get(`/api/announcements/${announcementId.value}`, {
...
const handleSubmit = async () => {
  ...
  // Validation
  let hasError = false;
  if (!form.title) { errors.title = 'Judul pengumuman wajib diisi'; hasError = true; }
  if (!form.content || form.content === '<p><br></p>') { errors.content = 'Isi pengumuman wajib diisi'; hasError = true; }
  if (!isEditMode.value && !imagePreview.value) { errors.url_image = 'Gambar sampul wajib diunggah'; hasError = true; }
  ...
  try {
    ...
    const endpoint = isEditMode.value ? `/api/announcements/${announcementId.value}` : '/api/announcements';
    ...
    if (response.data.status === 'success') {
      await Swal.fire({
        icon: 'success',
        title: 'Berhasil!',
        text: isEditMode.value ? 'Pengumuman berhasil diperbarui.' : 'Pengumuman baru berhasil ditambahkan.',
    ...
    const message = error.response?.data?.message || 'Terjadi kesalahan saat menyimpan pengumuman.';
    await Swal.fire({
      icon: 'error',
      title: 'Oops...',
      text: message,
