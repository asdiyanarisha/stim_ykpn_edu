        ...
        >
          <div v-if="!isAuthenticated || isLoading" class="absolute inset-0 z-[60] bg-slate-50/80 backdrop-blur-sm flex items-center justify-center">
              <div class="flex flex-col items-center">
                  ...
                  <p class="text-slate-500 font-medium animate-pulse">Memuat Detail Pengumuman...</p>
              </div>
          </div>
        </transition>

        <div v-if="announcement.id" class="max-w-4xl mx-auto space-y-8 pb-12">
          <!-- Announcement Header Area -->
          <div class="bg-white rounded-[2rem] shadow-sm border border-slate-100 overflow-hidden">
            <!-- Hero Image Section -->
            <div class="relative group">
                <div class="aspect-[21/9] w-full bg-slate-100 overflow-hidden">
                    <img v-if="announcement.url_image" :src="announcement.url_image" class="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" alt="Announcement Image">
                ...
                <div class="absolute top-6 left-6 flex gap-2">
                    <span 
                        ...
                        :class="announcement.status === 'published' ? 'bg-emerald-500/80 text-white border-emerald-400/50' : 'bg-slate-900/40 text-white border-white/20'"
                    >
                        {{ announcement.status }}
                    </span>
            ...
            <div class="p-8 md:p-12">
                ...
                <div class="flex flex-wrap items-center gap-6 mb-6 text-sm text-slate-400">
                    <div class="flex items-center gap-2">
                        <div class="w-8 h-8 rounded-full bg-indigo-50 flex items-center justify-center text-indigo-600 font-bold text-xs uppercase">{{ getInitials(announcement.creator_name || 'Admin') }}</div>
                        <span class="font-semibold text-slate-700">{{ announcement.creator_name || 'Administrator' }}</span>
                    </div>
                    <div class="flex items-center gap-2">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        {{ announcement.views_count }} Kali Dilihat
                    </div>
                </div>

                <h1 class="text-3xl md:text-4xl font-extrabold text-slate-900 leading-tight mb-8">{{ announcement.title }}</h1>
                
                <!-- Main Content Area -->
                <div class="prose prose-slate max-w-none text-slate-600 leading-relaxed news-content" v-html="announcement.content"></div>
            </div>
          </div>
          ...
          <div class="flex justify-between items-center bg-white p-6 rounded-[1.5rem] shadow-sm border border-slate-100">
            <div class="text-sm text-slate-400">
                Terakhir diperbarui: {{ formatDate(announcement.updated_at || announcement.created_at) }}
            </div>
