        ...
        <div class="max-w-4xl mx-auto mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <div class="flex items-center gap-2 text-sm text-slate-500 mb-2">
              <a href="/content/lowongan-kerja" class="hover:text-indigo-600 transition-colors">Manajemen Lowongan Kerja</a>
              <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" /></svg>
              <span class="text-slate-900 font-medium">{{ isEditMode ? 'Ubah Lowongan' : 'Tambah Lowongan' }}</span>
            </div>
            <h1 class="text-2xl font-bold text-slate-900">{{ isEditMode ? 'Ubah Informasi Lowongan' : 'Formulir Lowongan Baru' }}</h1>
            <p class="text-slate-500">{{ isEditMode ? 'Perbarui detail lowongan pekerjaan Anda.' : 'Tulis dan publikasikan lowongan kerja baru untuk STIM YKPN.' }}</p>
          </div>
        ...
        <div class="max-w-4xl mx-auto">
          <form @submit.prevent="handleSubmit" class="space-y-6">
            ...
            <div class="bg-white rounded-3xl p-8 shadow-sm border border-slate-100">
              ...
              <div class="space-y-6">
                <AppInput v-model="form.title" label="Judul Lowongan" placeholder="Misal: Dosen Pemasaran, Staf Akademik..." id="vacancy-title" :error="errors.title" />

                <!-- WYSIWYG Editor for Content -->
                <div class="space-y-1">
                  <label class="block text-sm font-bold text-slate-700 mb-1.5 ml-1">Detail Lowongan</label>
            ...
            <div class="bg-white rounded-3xl p-6 shadow-sm border border-slate-100 flex items-center justify-end gap-4">
              ...
              <AppButton variant="primary" size="md" type="submit" :disabled="isSubmitting">
                ...
                {{ isSubmitting ? (isEditMode ? 'Menyimpan...' : 'Menambahkan...') : (isEditMode ? 'Simpan Perubahan' : 'Simpan Lowongan') }}
              </AppButton>
            </div>
          </form>
        </div>
...
const goBack = () => {
  window.location.href = '/content/lowongan-kerja';
};
...
const fetchVacancyData = async () => {
    ...
    try {
        ...
        vacancyId.value = parts[parts.length - 1];
        const token = getCookie(TOKEN_COOKIE_NAME);
        
        const response = await axios.get(`/api/job-vacancies/${vacancyId.value}`, {
...
const handleSubmit = async () => {
  ...
  // Validation
  let hasError = false;
  if (!form.title) { errors.title = 'Judul lowongan wajib diisi'; hasError = true; }
  if (!form.content || form.content === '<p><br></p>') { errors.content = 'Detail lowongan wajib diisi'; hasError = true; }
  ...
  try {
    ...
    const endpoint = isEditMode.value ? `/api/job-vacancies/${vacancyId.value}` : '/api/job-vacancies';
    ...
    if (response.data.status === 'success') {
      await Swal.fire({
        icon: 'success',
        title: 'Berhasil!',
        text: isEditMode.value ? 'Lowongan berhasil diperbarui.' : 'Lowongan baru berhasil ditambahkan.',
    ...
    const message = error.response?.data?.message || 'Terjadi kesalahan saat menyimpan lowongan.';
    await Swal.fire({
      icon: 'error',
      title: 'Oops...',
      text: message,
